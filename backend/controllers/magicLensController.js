...
export const analyzeUploadedFile = async (req, res) => {
  ...
  try {
    ...
    // With multer.memoryStorage, we need to write the buffer to a temp file
    const safeName = fileName.replace(/[^a-zA-Z0-9_.-]/g, "_");
    tempFilePath = path.join(tempDir, `upload-${Date.now()}-${safeName}`);
...
export const analyzeFile = async (req, res) => {
  try {
    ...
    // Download Cloudinary file to a temporary path for analysis
    const urlObj = new URL(file.url);
    const ext = path.extname(urlObj.pathname) || "";
    const tempFilePath = path.join(tempDir, `magic-${file.id}-${Date.now()}${ext}`);

    console.log(`Analyzing file: ${file.name}`);
    console.log(`Cloudinary URL: ${file.url}`);
    console.log(`Temp file path: ${tempFilePath}`);
    ...
    await new Promise((resolve, reject) => {
      ...
      https.get(file.url, (response) => {
        if (response.statusCode !== 200) {
          return reject(new Error(`Failed to download file for analysis. Status: ${response.statusCode}`));
        }
    ...
    console.log(`Downloaded file size: ${fs.statSync(tempFilePath).size} bytes`);
    ...
    try {
      if (fs.existsSync(tempFilePath)) {
        fs.unlinkSync(tempFilePath);
        console.log(`Temporary file deleted after Magic Lens analysis: ${tempFilePath}`);
      }
    } catch (cleanupErr) {
      console.error(`Failed to delete temporary file after Magic Lens analysis: ${cleanupErr.message}`);
    }
