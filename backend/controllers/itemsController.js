...
export const duplicateItem = async (req, res) => {
    try {
        ...
        if (type === 'file') {
            ...
            // Duplicate only metadata; file.url already points to Cloudinary
            const fileExtensionIndex = file.name.lastIndexOf(".");
            const baseName = fileExtensionIndex === -1
                ? file.name
                : file.name.substring(0, fileExtensionIndex);
            const extension = fileExtensionIndex === -1
                ? ""
                : file.name.substring(fileExtensionIndex);
...
async function duplicateFolderContents(sourceFolderId, targetFolderId, userId) {
    ...
    // Duplicate all files by metadata only (Cloudinary URLs are reused)
...
export const deleteItem = async (req, res) => {
    try {
        ...
        if (type === 'file') {
            ...
            // Only delete database record; Cloudinary asset is managed via fileController.deleteFile
